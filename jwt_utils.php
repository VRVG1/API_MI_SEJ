...
function validate_jwt($jwt, $secret = 'secret')
{
    ...
    $header_encode = base64url_encode($header);
    // Codificando payload
    $payload_encode = base64url_encode($payload);
    // Creando la firma
    $signature_build = hash_hmac('SHA256', "$header_encode.$payload_encode", $secret, true);
    // Codificando la firma
    $signature_encode = base64url_encode($signature_build);
